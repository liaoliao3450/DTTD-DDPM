...
def generate_22ch_data(model, data_9ch, labels, device, data_scale_factor=1e5, guidance_scale=3.0):
    ...
    loader = DataLoader(
        TensorDataset(torch.FloatTensor(data_9ch_scaled), torch.LongTensor(labels)),
        batch_size=32,
        shuffle=False
    )
...
def within_subject_eval(data_path, model, device, data_scale_factor, subject_ids=range(1, 10), guidance_scale=3.0):
    print("\n" + "=" * 60)
    print("被试内测试 (Within-Subject) - 重构但不增强")
    print("=" * 60)
    ...
    for sid in subject_ids:
        ...
        accs_22, accs_9, accs_recon = [], [], []

        for train_idx, test_idx in skf.split(all_data_22ch, all_labels):
            ...
            gen_22ch = generate_22ch_data(
                model, train_9ch, train_labels, device, data_scale_factor, guidance_scale
            )

            clf = train_classifier(gen_22ch, train_labels, device, 22)
            metrics_recon = evaluate_classifier(clf, test_22ch, test_labels, device)
            accs_recon.append(metrics_recon['accuracy'])
            del clf
            torch.cuda.empty_cache()

        mean_22, std_22 = np.mean(accs_22), np.std(accs_22)
        mean_9, std_9 = np.mean(accs_9), np.std(accs_9)
        mean_recon, std_recon = np.mean(accs_recon), np.std(accs_recon)

        print(
            f"  22ch: {mean_22 * 100:.2f}%, "
            f"9ch: {mean_9 * 100:.2f}%, "
            f"Recon-only: {mean_recon * 100:.2f}%"
        )

        results[f'S{sid}'] = {
            'baseline_22ch': {'accuracy': mean_22, 'std': std_22},
            'baseline_9ch': {'accuracy': mean_9, 'std': std_9},
            'dttd_reconstructed_only': {'accuracy': mean_recon, 'std': std_recon}
        }
    ...
    avg_recon = np.mean([r['dttd_reconstructed_only']['accuracy'] for r in results.values()])
    results['average'] = {
        'baseline_22ch': avg_22,
        'baseline_9ch': avg_9,
        'dttd_reconstructed_only': avg_recon
    }

    print(
        f"\n平均: 22ch={avg_22 * 100:.2f}%, "
        f"9ch={avg_9 * 100:.2f}%, "
        f"Recon-only={avg_recon * 100:.2f}%"
    )
    return results
...
def cross_session_eval(data_path, model, device, data_scale_factor, subject_ids=range(1, 10), guidance_scale=3.0):
    print("\n" + "=" * 60)
    print("跨会话测试 (Cross-Session) - 重构但不增强")
    print("=" * 60)
    ...
    for sid in subject_ids:
        ...
        print(
            f"  22ch: {metrics_22['accuracy'] * 100:.2f}%, "
            f"9ch: {metrics_9['accuracy'] * 100:.2f}%, "
            f"Recon-only: {metrics_recon['accuracy'] * 100:.2f}%"
        )

        results[f'S{sid}'] = {
            'baseline_22ch': metrics_22,
            'baseline_9ch': metrics_9,
            'dttd_reconstructed_only': metrics_recon
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='DTTD仅重构分类评估实验')
    ...
    parser.add_argument('--output-dir', default='paper_results/reconstruction_only_classification')
    ...
    results = {
        ...
        'config': {
            ...
            'evaluation_type': 'reconstruction_only'
        }
    }
    ...
    output_file = os.path.join(args.output_dir, 'reconstruction_only_classification_results.json')
